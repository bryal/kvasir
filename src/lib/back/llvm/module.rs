...
use llvm_sys::bit_writer as writer;
use llvm_sys::bit_reader as reader;
    ...
    pub fn link(&self, src: &Module) -> Result<(), CBox<str>> {
        unsafe {
            ...
            if linker::LLVMLinkModules2(dest, src) == 1 {
                // TODO: Read error string from diagnostics
                Err(CBox::new(ptr::null_mut()))
            } else {
                Ok(())
            }
        }
