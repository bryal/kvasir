...
#[derive(Clone)]
pub struct ScopeStack<K, V>(Vec<BTreeMap<K, V>>);

impl<K: Ord + Eq, V> ScopeStack<K, V> {
    ...
    pub fn pop(&mut self) -> Option<BTreeMap<K, V>> {
        self.0.pop()
    }

    pub fn split_off(&mut self, at: usize) -> Vec<BTreeMap<K, V>> {
        self.0.split_off(at)
    }

    pub fn extend(&mut self, xs: Vec<BTreeMap<K, V>>) {
        self.0.extend(xs)
    }
...
impl<K: Debug + Ord + Eq, V: Debug> Debug for ScopeStack<K, V> {
